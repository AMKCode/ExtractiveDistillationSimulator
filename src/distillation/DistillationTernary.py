...
class DistillationModelTernary(DistillationModel):
    ...
    def plot_residue_curve_og(self, comp: np.ndarray, ax):
        data_points = 10
        init_comps = []
        ...
        for i in range(data_points):
            for j in range(data_points):
                if x1s[i, j] + x2s[i, j] > 1 or (x1s[i,j])**2 + (x2s[i,j])**2 < 0.4:
                    pass
                else:
    ...
    def plot_residue_curve_mod(self, comp: np.ndarray, ax):
        data_points = 15
        init_comps = []
        x1s, x2s = np.meshgrid(np.linspace(0, 1, data_points), 
                                np.linspace(0, 1, data_points))
        ...
        for i in range(data_points):
            for j in range(data_points):
                if x1s[i, j] + x2s[i, j] > 1 or (x1s[i,j])**2 + (x2s[i,j])**2 < 0.40:
                    pass
                else:
                    init_comps.append(np.array([x1s[i, j], x2s[i, j], 1 - x1s[i, j] - x2s[i, j]]))
        for init_comp in init_comps:
            def compute_residue_comp(init_comp):
                ...
                while True:
                    ...
                    x2 = x1 + delta_x
                    delta_y = (self.rectifying_step_xtoy(x2) - y1)*0.05
                    y2 = y1 + delta_y
