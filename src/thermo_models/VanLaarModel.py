...
class VanLaarModel(VLEModel):
    ...
    """
    def __init__(self, num_comp: int, P_sys: float, comp_names, partial_pressure_eqs: AntoineEquationBase10, A_coeff:dict):
...
class VanLaarBinaryModel(VLEModel):
    """
    A class representing a thermodynamic model based on Van Laar.
    ...
    Methods:
        ...
        convert_y_to_x(y, psat, A, B):
            Convert vapor mole fraction to liquid mole fraction based on Van Laar.
    """
    def __init__(self, num_comp: int, P_sys: float,  comp_names, partial_pressure_eqs: AntoineEquationBase10, A = None, B= None):
        super().__init__(num_comp, P_sys, comp_names)
        self.partial_pressure_eqs = partial_pressure_eqs
        self.A = A
        self.B = B
